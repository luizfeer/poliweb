...
function hasExclusionWord(query) {
...
export async function runCityAgent(input) {
    ...
    try {
        ...
        const directGarbageRows = await getDirectGarbageScheduleRows(input.supabase, input.cityId, input.query);
        ...
        const ferrySpecific = /águas verdes|aguas verdes|itapiché|itapiche|são francisco|sao francisco|\bitaci\b|campo do meio|ponte do itapiché|ponte do itapiche/i;
        if (ferryAskWords.test(input.query) && !ferrySpecific.test(input.query)) {
            ...
            if (!ferrySpecific.test(historyText)) {
                ...
                if (list.length > 1) {
                    const lines = list
                        .map((r) => `- ${r.short_name ?? r.name} (${r.endpoint_a_label ?? '?'} ⇄ ${r.endpoint_b_label ?? '?'})`)
                        .join('\n');
                    const text = `Temos ${list.length} travessias ativas no Lago de Furnas. Qual você quer ver?\n\n${lines}`;
                    const ferryCta = list.map((r, i) => ({
            ...
            try {
                ...
                if (Array.isArray(items) && items.length > 0) {
                    ...
                    const friendlyFallbacks = {
                        tourism_guide: 'Ainda não tenho guias de turismo cadastrados por aqui, mas posso te ajudar com outras coisas! Quer conhecer as atrações da região, ver onde ficar ou saber sobre os passeios de balsa?',
                        accommodation: 'Não encontrei hospedagens cadastradas no momento. Posso te mostrar atrações, restaurantes ou guias de turismo — o que prefere?',
                        restaurant: 'Não tenho restaurantes na lista agora. Quer ver onde ficar, o que fazer ou os eventos da cidade?',
                        attraction: 'Ainda não tenho atrações cadastradas. Posso te ajudar com hospedagem, restaurantes ou as balsas do Lago de Furnas.',
                        tour_package: 'Não encontrei pacotes turísticos por aqui. Quer saber das atrações, onde ficar ou os roteiros dos guias locais?',
                        fishing_guide: 'Ainda não tenho guias de pesca cadastrados. Quer conhecer os pontos de pesca ou as atrações da região?',
                        fishing_spot: 'Não encontrei pontos de pesca cadastrados. Posso te mostrar guias de turismo ou onde ficar.',
                        business: 'Não tenho comércios cadastrados no momento. Quer ver restaurantes, serviços ou atrações da cidade?',
                        property: 'Não encontrei imóveis cadastrados agora. Posso te ajudar com outras informações da cidade.',
                        event: 'Não tem eventos programados no momento. A agenda está vazia, mas posso te mostrar outras coisas!',
                        classified: 'Não encontrei classificados ativos. Quer ver comércios ou serviços da cidade?',
                        emergency_contact: 'Não tenho esses contatos cadastrados. Em emergência, ligue 192 (SAMU), 193 (Bombeiros) ou 190 (Polícia).',
                        health_facility: 'Não encontrei unidades de saúde cadastradas. Em emergência, procure o Pronto Socorro mais próximo.',
                    };
                    const blocks = [
                        {
                            type: 'text',
                            text: friendlyFallbacks[categoryType] ??
        ...
        const garbageContextWords = /\bcoleta\b|\blixo\b|res[íi]duo|recicl[áa]vel|org[âa]nico|descarte/i;
        const ferryContextWords = /\bbalsa\b|\bbalsas\b|travessia|travessias|ferry|itaci|itapiché|itapiche|águas verdes|aguas verdes|furnas|embarcaç|embarcac|\bbarca\b|lancha.*represa/i;
        if (hoursWords.test(input.query) &&
            ...
            try {
                ...
                if (Array.isArray(rows) && rows.length > 0) {
                    ...
                    if (Array.isArray(hoursData) && hoursData.length > 0) {
                        ...
                        const statusLabel = isOpenNow === true
        ...
        if (isExploratory) {
            systemParts.push('', 'PERGUNTA EXPLORATÓRIA: o usuário está pedindo um panorama (tipo "me conta sobre", "o que conhecer", "principais atrações", "como é tal lugar"). Use list_entities (NÃO search_entities) para puxar 5 a 8 itens da categoria certa. Depois ESCREVA narrativa em PT-BR: 3 a 6 frases em tom de guia local, organizadas por temas (natureza, cultura, gastronomia, etc), citando algumas das entidades retornadas pela ferramenta pelo nome. NÃO termine só com a lista — o texto narrativo é parte obrigatória da resposta. Se as ferramentas não retornarem nada relevante, escreva mesmo assim com conhecimento geral da região (geografia, contexto turístico de Furnas/Canastra) sem inventar entidades específicas.');
        }
        if (input.pageContext) {
            systemParts.push(`CONTEXTO DA PÁGINA ATUAL: ${input.pageContext}`);
        }
        systemParts.push('', 'Responda em PT-BR, de forma direta e conversacional.', '', 'REGRAS PARA QUANDO NÃO HOUVER RESULTADOS:', 'Se as ferramentas não retornarem resultados, NUNCA use tom de rejeição seca. Seja sempre acolhedor e ofereça alternativas CONCRETAS baseadas no que existe na cidade.', 'Exemplos de respostas boas:', '- "Ainda não tenho isso cadastrado, mas posso te ajudar com outras coisas! Quer saber sobre atrações, onde ficar, restaurantes ou os passeios de balsa?"', '- "Não encontrei exatamente isso, mas temos ótimas opções de pousadas, restaurantes e guias de turismo. Quer que eu liste alguma dessas?"', '- "Não tenho esse dado no momento. Posso te mostrar o que temos de melhor na cidade — atrações, hospedagem ou gastronomia?"');
        if (isFirst) {
            systemParts.push('', 'IMPORTANTE — TÍTULO DA CONVERSA: ao final da sua resposta, em uma linha separada, escreva exatamente no formato `[TITULO: <3 a 5 palavras em PT-BR resumindo o tópico>]`. Regras estritas: sem aspas, sem ponto final, sem emojis, sem asteriscos ou markdown ao redor (NÃO use `**[TITULO: ...]**`), sem nenhum texto depois do `]`. O marcador é OBRIGATÓRIO mesmo que a resposta seja curta. Esse marcador será removido antes de exibir.');
        }
        ...
        if (isExploratory && !hasFerry && toolBlocks.length > 0 && narrative.length < 80) {
            try {
                const entityNames = toolBlocks
                    .flatMap((b) => {
                    if (b.type === 'search_results')
                        return b.items.map((i) => i.name);
                    return [];
                })
                    .slice(0, 8);
        ...
        const hasResults = blocks.some((b) => b.type === 'search_results' ||
            b.type === 'entity_hours' ||
...
async function getDirectGarbageScheduleRows(supabase, cityId, query) {
    ...
    const rows = data.filter((row) => {
        ...
        if (!districts || typeof districts.name !== 'string')
            return false;
        const districtName = normalizeLocalText(districts.name);
        if (normalizedQuery.includes(districtName))
            return true;
        return districtName
            .split(/\s+/)
...
const ITINERARY_PREFERENCE_LABELS = {
...
async function maybeBuildItineraryResponse(input) {
    if (!isItineraryBuildQuery(input.query, input.conversation))
        return null;
    ...
    const [{ data: attractions, error: attractionsError }, { data: packages, error: packagesError }] = await Promise.all([
        input.supabase
            .from('attractions')
            .select('id, slug, name, type, description, address, difficulty, duration_minutes, cover_url, featured')
            .eq('city_id', input.cityId)
            .eq('status', 'published')
        ...
        input.supabase
            .from('tour_packages')
            .select('id, slug, title, description, duration_hours, itinerary, difficulty, cover_url, featured')
            .eq('city_id', input.cityId)
            .eq('status', 'published')
            .order('featured', { ascending: false })
    ...
    const blocks = [{ type: 'text', text }];
    if (cards.length > 0)
        blocks.push({ type: 'search_results', items: cards });
    return {
        blocks,
        fallback: false,
        usedTools: ['build_itinerary'],
...
function isItineraryBuildQuery(query, conversation) {
    ...
    const hasBuildWord = /\b(monta|montar|monte|planeja|planejar|planeje|cria|criar|crie|faz|fazer|faca|sugere|sugerir|sugira|personalizado|pra mim|para mim)\b/.test(normalized);
    const hasDuration = extractItineraryDays(query) !== null;
    return hasViagemWord && (hasBuildWord || hasDuration);
}
function extractItineraryDays(query) {
    const normalized = normalizeLocalText(query);
    ...
    if (/\b(meio dia|manha|tarde)\b/.test(normalized))
        return 1;
    if (/\b(um dia|1 dia|bate volta|bate-volta)\b/.test(normalized))
        return 1;
    if (/\b(fim de semana|final de semana|sabado e domingo)\b/.test(normalized))
        return 2;
    return null;
}
function extractItineraryPreferences(query) {
    const normalized = normalizeLocalText(query);
    const prefs = [];
    const add = (pref) => {
    ...
    if (/\b(lago|furnas|barco|lancha|marina|represa|agua|nautico)\b/.test(normalized))
        add('lake');
    ...
    if (/\b(comida|gastronomia|restaurante|almoco|jantar|cafe|doce|queijo)\b/.test(normalized))
        add('food');
    if (/\b(familia|crianca|leve|idoso|tranquilo|acessivel)\b/.test(normalized))
        add('family');
    if (/\b(aventura|radical|dificil|esporte|bike)\b/.test(normalized))
        add('adventure');
    if (/\b(pesca|pescaria|tucunare|peixe)\b/.test(normalized))
        add('fishing');
    return prefs;
}
function normalizeAttractionRows(value) {
...
function selectAttractionsForItinerary(attractions, preferences, days) {
...
function selectPackagesForItinerary(packages, preferences) {
    return packages
...
function scoreAttraction(item, preferences) {
    const text = normalizeLocalText([item.name, item.type, item.description, item.address, item.difficulty]
        .filter(Boolean)
        .join(' '));
    let score = item.featured ? 8 : 0;
    for (const pref of preferences)
        score += preferenceScore(text, pref);
    if (preferences.includes('family') && /dificil|radical|longa/.test(text))
        score -= 4;
    return score;
}
function scorePackage(item, preferences) {
    const text = normalizeLocalText([item.title, item.description, item.difficulty, JSON.stringify(item.itinerary)]
        .filter(Boolean)
        .join(' '));
    let score = item.featured ? 8 : 0;
    for (const pref of preferences)
...
function preferenceScore(text, pref) {
    const tests = {
...
function formatItineraryText(input) {
    ...
    if (input.packages.length > 0) {
        lines.push(`Roteiros prontos do portal que combinam com a ideia: ${input.packages.map((item) => item.title).join(', ')}.`, '');
    }
    const perDay = distributeByDay(input.attractions, input.days);
    perDay.forEach((dayItems, index) => {
        lines.push(`Dia ${index + 1}`);
        if (dayItems[0])
            lines.push(`Manhã: ${describeAttraction(dayItems[0])}`);
        if (dayItems[1])
            lines.push(`Tarde: ${describeAttraction(dayItems[1])}`);
        if (dayItems[2])
            lines.push(`Fim de tarde/noite: ${describeAttraction(dayItems[2])}`);
        if (input.preferences.includes('food')) {
            lines.push('Refeição: encaixe um restaurante local perto do trecho escolhido.');
        }
        lines.push('');
    });
    lines.push('Se quiser, eu ajusto o roteiro para ficar mais leve, mais econômico, com crianças, sem carro ou focado só no Lago de Furnas.');
    return lines.join('\n').trim();
}
function distributeByDay(attractions, days) {
...
function describeAttraction(item) {
    const parts = [item.name];
    if (item.type)
        parts.push(`perfil ${item.type}`);
    if (item.duration_minutes)
        parts.push(`reserve cerca de ${formatDuration(item.duration_minutes)}`);
    if (item.difficulty)
        parts.push(`dificuldade ${item.difficulty}`);
    return parts.join(' - ');
}
...
function itinerarySearchResults(attractions, packages) {
    ...
    const attractionItems = attractions.slice(0, 6).map((item) => ({
...
function formatToolResultBlocks(items) {
    ...
    if (items[0] && 'question' in items[0] && 'answer' in items[0]) {
        return [
            { type: 'faq', items: items.map((i) => ({ question: i.question, answer: i.answer })) },
        ];
    }
    // get_city_news
...
function groupGarbageScheduleRows(items) {
    ...
    for (const item of items) {
        ...
        const districtName = row.districts && typeof row.districts.name === 'string' ? row.districts.name : null;
    ...
    return Array.from(groups.values()).sort((a, b) => a.day_of_week - b.day_of_week || a.type.localeCompare(b.type));
}
...
function isExploratoryQuery(query) {
    const q = query.trim();
    if (q.length < 12)
        return false;
    return EXPLORATORY_PATTERNS.some((re) => re.test(q));
}
function makeFastPathTitle(actionType, query) {
    switch (actionType) {
        case 'faq':
...
function extractTitle(text) {
    let lastTitle = null;
    const stripped = text.replace(TITLE_TAG, (_, group) => {
        const t = group?.trim() ?? '';
        if (t)
            lastTitle = t;
        return '';
...
async function logAiJob(supabase, input) {
