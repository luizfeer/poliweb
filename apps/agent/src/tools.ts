...
import { formatStatusLabel, getStatus, type EntityHoursRow } from './hours.js';
...
export async function fetchEntityList(input: {
  ...
  if (HAS_STATUS[type]) query = query.eq('status', 'published');

  const ORDER: Record<EntityType, Array<[string, boolean]>> = {
...
export function createCityTools(input: { supabase: Supabase; env: AgentEnv; cityId: string }) {
  const { supabase, env, cityId } = input;

  return {
    search_entities: tool({
      description:
        'Busca entidades locais por significado semantico (embedding). Use para perguntas com conceito específico ("pousada perto do lago", "restaurante com vista", "trilha leve"). Para listar uma categoria inteira ("quero pousadas", "principais atrações", "guias de turismo") prefira list_entities.',
      ...
      execute: async ({ query, types }) => {
        ...
        const rows = (Array.isArray(data) ? (data as MatchEmbeddingRow[]) : []).filter(
          (row) => row.score >= MIN_SIMILARITY,
        );
    ...
    list_entities: tool({
      description:
        'Lista entidades públicas de uma categoria, sem usar embedding. Use para queries de browsing puro: "quais pousadas", "principais atrações", "guias de turismo", "restaurantes", "lista de pacotes". Mais confiável que search_entities quando a pergunta é categoria inteira.',
      inputSchema: z.object({
        type: z.enum(ALL_ENTITY_TYPES as [EntityType, ...EntityType[]]),
    ...
    get_entity_status: tool({
      ...
      execute: async ({ entity_type, entity_id, when }) => {
        if (!isEntityType(entity_type)) return { error: 'Tipo de entidade invalido.' };
        ...
        const [summary, hoursResult] = await Promise.all([
          ...
          supabase
            .from('entity_hours')
            .select(
              'weekday, starts_at, ends_at, kind, valid_from, valid_until, source_status, active',
            )
            .eq('city_id', cityId)
        ...
        const status = getStatus(
          Array.isArray(hoursResult.data) ? (hoursResult.data as EntityHoursRow[]) : [],
    ...
    get_entity_faq: tool({
      ...
      execute: async ({ entity_type, entity_id, question }) => {
        ...
        if (!question) return faqs.slice(0, 5);

        const normalized = question.toLowerCase();
        return faqs
          .filter((faq) => {
            const q =
              'question' in faq && typeof faq.question === 'string'
    ...
    get_emergency_contacts: tool({
      description:
        'Retorna telefones de emergência e utilidade pública: SAMU, Bombeiros, Defesa Civil, Polícia, Copasa, Cemig, Prefeitura, etc.',
      inputSchema: z.object({
    ...
    get_pharmacy_on_duty: tool({
      description:
        'Consulta qual farmácia está de plantão hoje ou em uma data específica em Carmo do Rio Claro.',
      inputSchema: z.object({
      ...
      execute: async ({ date }) => {
        ...
        const { data, error } = await supabase
          ...
          .select(
            'shift_type, notes, start_date, end_date, pharmacies(name, address, phone, whatsapp, is_24h, google_maps_url)',
          )
    ...
    get_garbage_schedule: tool({
      description:
        'Retorna o calendário de coleta de lixo por bairro/distrito. Se bairro não informado, retorna todos.',
      inputSchema: z.object({
      ...
      execute: async ({ district_name, day_of_week }) => {
        ...
        if (day_of_week !== undefined) query.eq('day_of_week', day_of_week);
    ...
    get_service_alerts: tool({
      description:
        'Busca alertas ativos de serviços: falta de água, corte de energia, trânsito, clima extremo, etc.',
      inputSchema: z.object({
    ...
    get_church_schedule: tool({
      description:
        'Busca igrejas e horários de missas, cultos e reuniões em Carmo do Rio Claro. Retorna o calendário semanal completo de cada igreja, com indicação de quais itens são hoje. Use para qualquer pergunta sobre missa, culto, celebração, igrejas.',
      inputSchema: z.object({
      ...
      execute: async ({ tradition, church_name }) => {
        ...
        if (tradition) churchQuery = churchQuery.eq('tradition', tradition);
        if (church_name) churchQuery = churchQuery.ilike('name', `%${church_name}%`);
        ...
        for (const s of (Array.isArray(schedules) ? schedules : []) as any[]) {
          if (!scheduleMap.has(s.church_id)) scheduleMap.set(s.church_id, []);
        ...
        return churches
          .map((c: any) => ({
            ...
            has_today: (scheduleMap.get(c.id) ?? []).some((s: any) => s.weekday === todayWeekday),
          }))
          .filter((c: any) => c.weekly_schedule.length > 0);
      },
    }),

    get_ferry_info: tool({
      description:
        'Consulta horários, valores e avisos das balsas/travessias do Lago de Furnas (Itaci, Itapiché, Águas Verdes etc.). Use para qualquer pergunta sobre balsa, travessia, ferry boat, horário de balsa, preço de balsa.',
      inputSchema: z.object({
        route_query: z
          .string()
          .optional()
      ...
      execute: async ({ route_query, direction, next_only }) => {
        let routeQuery = supabase
          .from('ferry_routes')
          .select(
            'id, slug, name, short_name, region, district, status, confidence, description, important_info, fare_summary, fare_warning, fare, endpoint_a_label, endpoint_b_label, related_cities, operating_days, source, featured',
          )
          .eq('city_id', cityId)
          .eq('active', true)
          .order('display_order', { ascending: true });
        if (route_query) routeQuery = routeQuery.ilike('name', `%${route_query}%`);
        const { data: routes, error: routeError } = await routeQuery.limit(5);
        if (routeError) throw new Error(routeError.message);

        const list = Array.isArray(routes) ? routes : [];
        if (list.length === 0) return { error: 'Nenhuma balsa encontrada.' };
        ...
        if (direction) scheduleQuery = scheduleQuery.ilike('direction', `%${direction}%`);
        ...
        const allSchedules = (Array.isArray(schedules) ? schedules : []) as Array<{
          route_id: string;
          direction: string;
          origin: string | null;
        ...
        const allAlerts = (Array.isArray(alerts) ? alerts : []) as Array<{
          route_id: string;
          type: string;
          title: string;
          message: string;
        ...
        return list.map((r: any) => {
          ...
          for (const dir of Object.keys(grouped)) {
            ...
            if (nextItem) nextItem.isNext = true;
            if (next_only) grouped[dir] = nextItem ? [nextItem] : [];
          }
          return {
            slug: r.slug,
