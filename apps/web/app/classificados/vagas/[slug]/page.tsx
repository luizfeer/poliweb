...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!classified || classified.type !== 'job') return { title: 'Vaga - Classificados' };
  return {
...
export default async function JobClassifiedPage({ params }: { params: Promise<{ slug: string }> }) {
  const city = await getCurrentCity();
  if (!city || !city.modules.includes('classifieds')) notFound();
  const { slug } = await params;
  const classified = await getClassifiedBySlug({ cityId: city.id, slug });
  if (!classified || classified.type !== 'job') notFound();
