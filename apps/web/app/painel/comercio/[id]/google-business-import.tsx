...
import {
  applyGoogleBusinessImportAction,
  getGoogleBusinessDetailsAction,
...
type GoogleBusinessImportProps = {
  businessId: string;
  defaultQuery: string;
};
...
const FIELD_OPTIONS = [
  ...
  { value: 'payment_methods', label: 'Pagamentos' },
  { value: 'price', label: 'Faixa de preco' },
] as const;

type FieldName = typeof FIELD_OPTIONS[number]['value'];

export function GoogleBusinessImport({ businessId, defaultQuery }: GoogleBusinessImportProps) {
  ...
  const availableFields = useMemo(() => {
    ...
    return FIELD_OPTIONS.filter((field) => {
      ...
      if (field.value === 'payment_methods') return details.paymentMethods.length > 0;
  ...
  function searchCandidates() {
    ...
    startTransition(async () => {
      ...
      setDetails(null);
      setSelectedReviews([]);
      const result = await searchGoogleBusinessCandidatesAction({ businessId, query });
  ...
  function loadDetails(placeId: string) {
    startTransition(async () => {
      ...
      const result = await getGoogleBusinessDetailsAction({ businessId, placeId });
      if (result.details) {
  ...
  function applyImport() {
    ...
    startTransition(async () => {
      setMessage('Aplicando os campos e importando fotos para a galeria...');
      setPendingLabel('Aplicando importacao...');
      const result = await applyGoogleBusinessImportAction({
        businessId,
    ...
    <div className="grid gap-3">
      <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
        <input
          ...
          placeholder="Nome do comercio, endereco ou ponto de referencia"
        />
        <button
          type="button"
          ...
          >
            <header className="flex items-start justify-between gap-3 border-b border-ink-100 bg-white p-4">
              <div className="min-w-0">
                <h2 id="google-import-title" className="text-base font-semibold">
                  Importar dados do Google
                </h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  Aprove apenas o que deve entrar na ficha. Fotos selecionadas entram na galeria.
                </p>
              </div>
              <button
                type="button"
              ...
              {details ? (
                <div className="mt-3 grid gap-3 rounded-xl border border-ink-100 bg-white p-3">
                  ...
                  {details.photos.length > 0 ? (
                    <fieldset className="grid gap-2">
                      <legend className="text-sm font-semibold">Fotos para importar</legend>
                      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
  ...
  return (
    <section className="grid gap-2 rounded-lg border border-ink-100 bg-paper p-3">
      ...
      <div className="grid gap-3 text-sm md:grid-cols-2">
        ...
        <PreviewBlock title="Horarios extras" items={details.secondaryHours} empty="Sem delivery/retirada separado." />
        <PreviewBlock title="Comodidades" items={details.attributes.map((item) => `${item.label}: ${item.value ? 'sim' : 'nao'}`)} empty="Sem caracteristicas extras." />
        <PreviewBlock title="Pagamentos" items={details.paymentMethods} empty="Sem pagamentos no Google." />
      </div>
      {details.summaries.length > 0 ? (
        <div className="grid gap-2">
          {details.summaries.map((summary) => (
            <article key={summary.kind} className="rounded-md bg-white p-2">
              <p className="text-xs font-semibold text-muted-foreground">{summary.label}</p>
    ...
    <fieldset className="grid gap-2">
      ...
      {reviews.length > 0 ? (
        ...
        <p className="rounded-lg bg-paper px-3 py-2 text-sm text-muted-foreground">
          O Google nao retornou comentarios para este lugar. Pode ser limite da API, perfil sem texto ou permissao/billing do Places.
        </p>
      )}
    </fieldset>
  );
