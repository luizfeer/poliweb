<template>
  <q-page class="bg-gray-100">
    <div class="bg-white border-b border-gray-200 p-3 mb-3">
      <div class="flex flex-nowrap">
        <!-- <div class="h-20 w-20 min-w-[5rem] rounded-full overflow-hidden">
          <q-img
            src="/thumb.png"
            :ratio="1"
            class="h-full w-full"
            spinner-color="white"
            spinner-size="82px"
          />
        </div> -->
        <div class="pl-3">
          <h1 class="text-2xl text-gray-700 font-semibold">
           {{ data.name }}
          </h1>
          <h2 class="text-lg text-gray-500">{{ data.description }}</h2>
        </div>
      </div>
      <q-space />
      <div class="flex items-center justify-between mt-4">
          <a v-if="phoneZap" :href="`https://api.whatsapp.com/send?phone=${phoneZap.phone}&text=Ol%C3%A1!`" target="_blank" rel="noopener noreferrer">
            <q-btn push color="positive" text-color="white">
                <q-icon name="fab fa-whatsapp" class="mr-2" /> Whatsapp
            </q-btn>
          </a>
        <q-btn
          :push="!follow"
          :color="!follow ? 'white' : 'positive'"
          :text-color="follow ? 'white' : 'blue-grey-8'"
          :label="follow ? 'Seguindo' : 'Seguir'"
          @click="follow = !follow"
        />
      </div>
    </div>

    <vue-picture-swipe
        v-if="data.photos"
      :options="{ bgOpacity: 0.65 }"
      :items="items"
    ></vue-picture-swipe>
    <div class="p-3">
      <div class="bg-white border border-gray-200 rounded-md p-3">
        <h1 class="text-xl text-gray-700 font-semibold">Sobre</h1>
        <p class="text-gray-600">
          {{ data.description }}
        </p>
      </div>
      <div class="bg-white border border-gray-200 rounded-md p-3 text-lg mt-3">
        <div v-for="(phone, index) in data.phones" :key="phone.id">
            <template v-if="!phone.isWhatsapp">
                <a :href="`tel:${phone.phone}`" class="flex items-center flex-nowrap text-gray-600">
                    <q-icon
                        name="fas fa-phone-square-alt"
                        class="mr-2 text-xl text-red-600"
                    />
                    {{ this.phone(phone.phone) }}
                </a>
                <div v-if="index !== data.phones.length-1" class="divider border-t border-gray-200 w-full px-5 my-3"></div>
            </template>
            <template  v-else>
                <a  :href="`https://api.whatsapp.com/send?phone=${phone.phone}&text=Ol%C3%A1!`" target="_blank" rel="noopener noreferrer" class="flex items-center flex-nowrap text-gray-600">
                    <q-icon
                        name="fab fa-whatsapp-square"
                        class="mr-2 text-xl text-green-600"
                    />
                    {{ this.phone(phone.phone) }}
                </a>
            <div class="divider border-t border-gray-200 w-full px-5 my-3"></div>
            </template>
        </div>
        </div>

      <div class="bg-white border border-gray-200 rounded-md p-3 text-xl mt-3">
        <div v-if="data.site" class="flex items-center flex-nowrap text-gray-600">
            <a :href="`https://:${data.site}`" target="_blank" rel="noopener noreferrer">
                <q-icon name="language" class="mr-2 text-xl text-blue-400" />
                {{ data.site }}
            </a>
        </div>

        <div v-if="data.site && (data.facebook || data.instagram || data.email)" class="divider border-t border-gray-200 w-full px-5 my-3"></div>

        <div v-if="data.facebook" class="flex items-center flex-nowrap text-gray-600">
            <a :href="`https://:${data.facebook}`" target="_blank" rel="noopener noreferrer">
                <q-icon
                    name="fab fa-facebook-square"
                    class="mr-2 text-xl text-blue-800"
                />
                {{ data.facebook }}
            </a>
        </div>
        <div v-if="data.facebook  && (data.instagram || data.email)" class="divider border-t border-gray-200 w-full px-5 my-3"></div>

        <div v-if="data.instagram" class="flex items-center flex-nowrap text-gray-600">
            <a :href="`https://:${data.instagram}`" target="_blank" rel="noopener noreferrer">
                <q-icon
                    name="fab fa-instagram-square"
                    class="mr-2 text-xl text-pink-600"
                />
                {{ data.instagram }}
            </a>
        </div>
        <div v-if="data.instagram && data.email" class="divider border-t border-gray-200 w-full px-5 my-3"></div>
        <div v-if="data.email" class="flex items-center flex-nowrap text-gray-600">
            <a :href="`mail:${data.email}`" target="_blank" rel="noopener noreferrer">
                <q-icon
                    name="fas fa-envelope-open-text"
                    class="mr-2 text-xl text-purple-800"
                />
                {{ data.email }}
            </a>
        </div>
      </div>
      <div class="bg-white border border-gray-200 rounded-md p-3 text-xl mt-3">
          <a :href="`http://maps.google.com/maps?q=${data.name},${data.addresses[0].street},Nº${data.addresses[0].number},${data.addresses[0].city} ${data.addresses[0].state},${data.addresses[0].zipCode}`" target="_blank" rel="noopener noreferrer">
        <div
          class="
            flex flex-col
            justify-center
            items-center
            flex-nowrap
            text-gray-600
          "
        >
          <q-icon
            name="fas fa-map-marked-alt"
            class="mr-2 text-3xl text-yellow-500"
          />
          {{ `${data.addresses[0].street}, Nº${data.addresses[0].number}. ${data.addresses[0].city} ${data.addresses[0].state} - ${data.addresses[0].zipCode}` }}
        </div>
        </a>
      </div>
    </div>
  </q-page>
</template>

<script>
import { defineComponent } from "vue";
import { Vue, ref } from "vue";
import VuePictureSwipe from "vue-picture-swipe";

export default defineComponent({
  name: "Categorias",
  components: {
    VuePictureSwipe,
  },
  setup() {
    return {
      slide: ref(1),
      follow: ref(false),
      data: ref({
        id: 701,
        categoryId: 501,
        customerId: 301,
        description: "Caloroso restaurante com pratos finos.",
        email: null,
        facebook: "https://facebook.com/joquinhaenterprise",
        instagram: null,
        name: "Restaurante do Joquinha",
        website: null,
        createdAt: "2021-07-24T00:29:19.320Z",
        updatedAt: null,
        deletedAt: null,
        phones: [   {
            id: 1001,
            categoryAdId: 701,
            isWhatsapp: false,
            phone: "51998964837",
            createdAt: "2021-07-24T03:55:21.431Z",
            deletedAt: null
        },
        {
            id: 1002,
            categoryAdId: 701,
            isWhatsapp: true,
            phone: "357779899898",
            createdAt: "2021-07-24T03:55:32.016Z",
            deletedAt: null
        },
        {
            id: 1003,
            categoryAdId: 701,
            isWhatsapp: false,
            phone: "357779899898",
            createdAt: "2021-07-24T03:55:36.589Z",
            deletedAt: null
        }
    ],
    addresses: [
    {
        id: 1101,
        categoryAdId: 701,
        neighborhood: "Centro",
        zipCode: "37860000",
        city: "Nova Resende",
        state: "MG",
        country: "Brasil",
        coordinates: {
        lat: -21.114253,
        long: -46.409309,
        rawType: true
            },
        street: "Nova Resende",
        complement: null,
        number: 1,
        createdAt: "2021-07-24T03:56:10.779Z",
        deletedAt: null
        }
      ]
    }),
    phone(phone) {
        return phone.replace(/[^0-9]/g, '')
                    .replace(/(\d{2})(\d{5})(\d{4})/, '($1) $2-$3');
        },

      
    //   items: [
    //     {
    //       src: "https://images.unsplash.com/photo-1607868894760-4241c06dd023?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80",
    //       thumbnail:
    //   "https://images.unsplash.com/photo-1607868894760-4241c06dd023?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80",
    //       w: 634,
    //       h: 951,
    //       alt: "some numbers on a grey background", // optional alt attribute for thumbnail image
    //     },
    //     {
    //       src: "https://images.unsplash.com/photo-1565632444383-283cfa0dbe62?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=967&q=80",
    //       thumbnail:
    //  "https://images.unsplash.com/photo-1565632444383-283cfa0dbe62?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=967&q=80",
    //       w: 1200,
    //       h: 900,
    //     },
    //     {
    //       src: "https://images.unsplash.com/photo-1607868894760-4241c06dd023?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80",
    //       thumbnail:
    //         "https://images.unsplash.com/photo-1607868894760-4241c06dd023?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80",
    //       w: 634,
    //       h: 951,
    //       alt: "some numbers on a grey background", // optional alt attribute for thumbnail image
    //     },
    //     {
    //       src: "https://images.unsplash.com/photo-1565632444383-283cfa0dbe62?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=967&q=80",
    //       thumbnail:
    //         "https://images.unsplash.com/photo-1565632444383-283cfa0dbe62?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=967&q=80",
    //       w: 1200,
    //       h: 900,
    //     },
    //   ],
    };
  },
  computed: {
      phoneZap() {          
          for (let index = 0; index < this.data.phones.length; index++) {
              const element =  this.data.phones[index];
              if(element.isWhatsapp)
              {
                  return element
              }              
          }
          return false
      }
  },
});
</script>
<style>
.my-gallery {
  width: 100%;
  display: flex;
  flex-wrap: nowrap;
  overflow-x: auto;
  overflow-y: hidden;
  padding-left: 0.75rem;
  overflow-scrolling: touch;
  webkit-overflow-scrolling: touch;
}

.my-gallery img {
  width: 100%;
  height: auto;
  object-fit: cover;
  min-height: 150px;
  min-width: 150px;
}

.my-gallery figure {
  display: flex;
  margin: 0 !important;
  margin-right: 0.75rem !important;
  width: 150px;
  min-width: 150px;
  height: 150px;
  overflow: hidden;
  border-radius: 0.5rem;
}

.my-gallery figcaption {
  display: none;
}
</style>
