...
export type RouterAction =
  ...
  | { type: 'news'; limit: number }
  | { type: 'events'; limit: number }
  | { type: 'search'; query: string; types?: string[] }
  | { type: 'llm' }; // deixa o LLM decidir com tools

const KEYWORD_MAP: Array<{ keywords: string[]; action: RouterAction; exclude?: RegExp }> = [
  // FAQ da plataforma
  {
    keywords: [
      'cadastrar',
  ...
  // Eventos
  {
    ...
    exclude:
      /\baluga|aluguel|comprar|vender|preço|preco|quanto|custa|barato|caro|loja|comercio|comércio|vestido|mesa|cadeira|decoração|decoracao|buffet|som|dj|iluminação|iluminacao|foto|filmagem|salão|salao|bolo|doce|salgado|carro|moto|van|frete|mudança|mudanca|equipamento|ferramenta|inflável|inflavel|brinquedo|pula-pula/i,
  },
];
...
let faqEmbeddings: Array<{ question: string; embedding: number[] }> | null = null;

/** Preenche embeddings do FAQ no startup (lazy). */
export async function initRouter(env: AgentEnv): Promise<void> {
  if (faqEmbeddings) return;
  const embeddings = await Promise.all(
    FAQ_QUESTIONS.map(async (q) => ({
      question: q,
...
export async function routeQuery(query: string, env: AgentEnv): Promise<RouterAction> {
  const normalized = query.toLowerCase();

  // 1. Keywords hardcoded (rápido, barato, 100% consistente)
  for (const map of KEYWORD_MAP) {
    if (map.exclude && map.exclude.test(normalized)) continue;
