...
const REFERENCE_TOKENS = /\b(ela|ele|esse|essa|isso|aquele|aquela|aquilo|l[áa]|a[íi]|esse lugar|esse local)\b/i;
export function isCacheable(input) {
    if (!input.isFirstMessage)
        return false;
    if (input.hasConversation)
        return false;
    if (REFERENCE_TOKENS.test(input.query))
        return false;
    return true;
}
export function cacheKey(citySlug, query) {
...
export function setCached(key, response) {
 ...
 */
export async function dedupeInflight(key, loader) {
