import type { CtaButton } from './response-types.js';

type CtaRule = {
  /** Regex que casa contra a query do usuário (lowercase, sem normalização). */
  match: RegExp;
  buttons: CtaButton[];
};
...
const RULES: CtaRule[] = [
  ...
  {
    match:
      /\brestaurante|delivery|comida|comer|almoçar|jantar|lanche|pizza|pizzaria|pão|padaria|sorvete|bar\b|açougue|mercado|sacolão|conveniência/i,
  ...
  {
    match:
      /\bevento|festa|show|agenda|cultural|fim de semana|sábado|domingo|forró|sertanejo|rodeio/i,
...
const REFERRAL_RULES: ReferralRule[] = [
  ...
  {
    match:
      /\bvestido|terno|fantasia|roupa|decoração|buffet|som|dj|foto|filmagem|salão.*festa|bolo|doce|salgado|mesa|cadeira|talher/i,
    categoryLabel: 'eventos e festas',
  },
];

export function ctaForQuery(query: string): CtaButton[] {
  const out: CtaButton[] = [];
  const seen = new Set<string>();
  for (const rule of RULES) {
    if (!rule.match.test(query)) continue;
    for (const btn of rule.buttons) {
      if (seen.has(btn.href)) continue;
      seen.add(btn.href);
      out.push(btn);
    }
  }
 ...
 */
export function referralCtaForQuery(query: string, cityName: string): CtaButton | null {
  const normalized = query.toLowerCase();

  // Se tem palavra de exclusão, não sugere CTA de comércio
  if (REFERRAL_EXCLUSIONS.some((w) => normalized.includes(w))) return null;

  for (const rule of REFERRAL_RULES) {
    if (rule.match.test(normalized)) {
      const text = encodeURIComponent(
        `Oi! Vi que tem um portal sobre ${cityName} e pensei em recomendar vocês. Tem como se cadastrar lá?`,
      );
      return {
