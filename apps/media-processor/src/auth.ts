...
export type AuthResult =
  | { kind: 'static' }
  | { kind: 'token'; payload: UploadTokenPayload };

export function authorize(headerValue: string | undefined): AuthResult {
  if (!headerValue) {
...
function verifyToken(token: string): UploadTokenPayload {
  ...
  if (!payloadB64 || !sigB64) throw new HttpError(401, 'malformed token');
...
function signString(input: string, secret: string): string {
